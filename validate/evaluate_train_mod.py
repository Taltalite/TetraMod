#!/usr/bin/env python3
"""
Evaluate a TetraMod train checkpoint on basecalling and modification targets.
...
from tetramod.util import accuracy, decode_ref, init, load_model

try:
    from train_mod_common import maybe_trim_refs, resolve_train_mod_data_settings
except ImportError:
    from validate.train_mod_common import maybe_trim_refs, resolve_train_mod_data_settings
...
def resolve_data_settings(args) -> DataSettings:
    return resolve_train_mod_data_settings(
...
def argparser():
    ...
    parser.add_argument("--chunks", type=int, default=None, help="Same meaning as tetramod train --chunks when the dataset has no validation/ directory.")
    parser.add_argument("--valid-chunks", type=int, default=None, help="Same meaning as tetramod train --valid-chunks when the dataset has no validation/ directory.")
